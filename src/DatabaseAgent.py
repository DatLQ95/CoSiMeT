...
class DatabaseAgent:
    def __init__(self):
        # FIXME: we have not deal with the case that we dont habe database in
        # the first place, let's assume we already have the database named:
        # CloudSightServers
        self.mydb = None
        self.fernet = None
    ...
    def check_crypto_key(self):
        ...
        mycursor.execute(sql)
        myresult = mycursor.fetchall()
        current_hash = hashlib.sha224(
            src.config.general_info['crypto_key'].encode()).hexdigest()
        ...
        for x in myresult:
            if bool(x[2]):
                cs_server = CloudSightServer(
                    name=x[0],
                    url=x[1],
                    key=self.fernet.decrypt(
                        x[2]),
                    remote_user=x[3],
                    status=x[4],
                    version=x[5],
                    last_time_update_info=x[6],
                    certi_expiry_date=x[7],
                    access_port=x[8],
                    certi_issuer=x[9],
                    crypto_method=x[10],
                    ssh_password=x[11])
                ...
                cs_server = CloudSightServer(
                    ...
                    certi_issuer=x[9],
                    crypto_method=x[10],
                    ssh_password=self.fernet.decrypt(
                        x[11].encode()).decode("utf-8"))
            else:
                # TODO: handle the case
                #  Print out the the key is not valid. 
                cs_flag_valid = False
                pass
            if cs_flag_valid: 
    ...
    def check_if_server_exist(self, cs_server):
        ...
        mycursor.execute(
            f"SELECT * FROM {self.table_name} WHERE name ='{cs_server.get_name()}'")
        for x in mycursor:
            if x:
                exist = True
        return exist
    ...
    def encrypt_ssh_key(self, data):
        ...
        for cs_server_list in data:
            if bool(cs_server_list[2]):
                cs_server_list[2] = self.fernet.encrypt(cs_server_list[2])
            if bool(cs_server_list[11]):
                cs_server_list[11] = self.fernet.encrypt(
                    cs_server_list[11].encode())
        return tuple(cs_server_list)

    def update_CS_servers(self, cs_server_list):
        mycursor = self.mydb.cursor()

        sql = f"INSERT INTO {self.table_name} (name, url , server_ssh_key , remote_use , status, version , date, expiry_date, port, certi_issuer, crypto_method, ssh_password) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    ...
    def update_general_info(self):
        ...
        # Add the hash:
        data.append(("crypto_key_hash", hashlib.sha224(
            src.config.general_info['crypto_key'].encode()).hexdigest()))
        print(data)
        mycursor.executemany(sql, data)
    ...
    def get_general_info(self):
        ...
        for item in myresult:
            if item[0] in src.config.general_info.keys():
                src.config.general_info[item[0]] = self.fernet.decrypt(
                    item[1].encode()).decode("utf-8")
    ...
    def get_key(self):
        ...
        kdf = PBKDF2HMAC(
            ...
            length=32,
            salt=src.config.general_info['salt'].encode(),
            iterations=100,
            backend=default_backend())
        key = base64.urlsafe_b64encode(kdf.derive(
            src.config.general_info['crypto_key'].encode()))
        return key
